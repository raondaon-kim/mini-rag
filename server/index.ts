...
await mkdir("data/memory/intents", { recursive: true });
await mkdir("data/memory/conversations", { recursive: true });
await mkdir("data/memory/task-log", { recursive: true });
...
app.use("/api/files", express.static(path.resolve("data/output")));
...
app.listen(PORT, () => {
  ...
  console.log(`   👁  Watching ${DOCS_PATH}/ for auto-indexing\n`);
});
